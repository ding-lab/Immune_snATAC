...
#Read RDS object:
obj_downs=readRDS('PanImmune_RNA_ATAC.v8.0.CD8.500cells_perCellType.rds')
...
pdf(paste('plots/CD8_cell_obj_ATAC_500_cells_perCelltype_', n_components,
'_maxComp.',run_date,'.pdf',sep=''),width=9,height=7,useDingbats=F)
print(p1)
dev.off()

saveRDS(monocle_cds,paste('Monocle_RDS/CD8_cell_obj_ATAC_500_cells_perCelltype_', n_components,
'_maxComp.',run_date,'.rds',sep=''))
...
monocle_cds=readRDS(paste('Monocle_RDS/CD8_cell_obj_ATAC_500_cells_perCelltype_10_maxComp.',run_date,'.rds',sep=''))

p1=plot_cell_trajectory(monocle_cds, color_by = "Cancer", cell_size = 1) +
scale_color_manual(values=dis_cols)

pdf(paste('plots/CD8_obj_ATAC_500_cells_perCelltype_', n_components,
...
write.table(tab_s,paste('Monocle_RDS/All_PTs_monocle_objs.10comp.CD8.',run_date,'.tsv',sep=''),sep='\t',quote=F,row.names=F)
...
pdf(paste('plots/CD8_obj_ATAC_500_cells_perCelltype_', n_components,
'_maxComp_byPseudotime.',run_date,'.pdf',sep=''),width=9,height=7,useDingbats=F)
print(p1)
dev.off()

p1=plot_cell_trajectory(monocle_cds, color_by = "cell_type", cell_size = 1)
tab <- ggplot_build(p1)[["plot"]][["data"]]
tab_s=tab[,c('sample_name','data_dim_1', 'data_dim_2','Pseudotime','cell_type', 'sample_state','State')]

#Get the state for naive, its 16
table(tab_s$State[tab_s$cell_type=='CD8 T-cells naive'])

#Re-order cells 
monocle_cds <- orderCells(monocle_cds, root_state=16)

saveRDS(monocle_cds,paste('Monocle_RDS/CD8_cell_obj_ATAC_500_cells_perCelltype_', n_components,
'_maxComp.rootState_16.',run_date,'.rds',sep=''))

p1=plot_cell_trajectory(monocle_cds, color_by = "Pseudotime", cell_size = 1) +
scale_color_viridis(option="B")

pdf(paste('plots/CD8_obj_ATAC_500_cells_perCelltype_', n_components,
'_maxComp_byPseudotime.rootState_16.',run_date,'.pdf',sep=''),width=9,height=7,useDingbats=F)
print(p1)
dev.off()


#Save new PT values to the file:

p1=plot_cell_trajectory(monocle_cds, color_by = "cell_type", cell_size = 1)
tab <- ggplot_build(p1)[["plot"]][["data"]]
tab_s=tab[,c('sample_name','data_dim_1', 'data_dim_2','Pseudotime','cell_type', 'sample_state','State')]

write.table(tab_s,paste('Monocle_RDS/All_PTs_monocle_objs.10comp.CD8.After_reOrdering.state16_root.',run_date,'.tsv',sep=''),
